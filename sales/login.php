...
if(isset($_POST) && count($_POST) > 0){
  ...
  $sql = "SELECT * FROM sales_signup WHERE user = '".$email."' AND password = '".$password."'";
  $result = mysqli_query($conn, $sql);

  if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)){
      $_SESSION['salesuser'] = $row['user'];
      $_SESSION['salespass'] = $row['password'];
      $_SESSION['salesid'] = $row['id'];
      header('Location: https://dashboard.selectdigitizing.com/sales/index.php');
    }
  }
  else{
