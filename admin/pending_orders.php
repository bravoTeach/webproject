...
// -------------------------------
$sql2="SELECT * FROM placeorder WHERE status = 'in process'";
...
?>
<div>
<h4>Total Pending Orders:  <?php echo $inquiry;  ?> </h4>
...
<td>  <a href="download.php?id=<?= $row["id"];?>">  <img src="<?php echo $imageURL; ?>" alt="" height="50px" width="50px"/></a></td>


<td> <a href="download_order1.php?id=<?= $row["id"];?>">    <img src="<?php echo $imageURL1; ?>" alt="" height="50px" width="50px"/></a></td>
...
<td ><a   href="release_order.php?id=<?= $row["id"];?>" style="background-color:blue;">RELEASE</a></td>
<td><a href="pdf_order.php?id=<?= $row['id'] ?>" onclick="generatePDF(event, <?= $row['id'] ?>);" style="background-color: blue;">PDF</a></td>

<script>
function generatePDF(event, orderId) {
    event.preventDefault(); // prevent the link from navigating to the PDF file directly

    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'pdf_order.php?id=' + orderId); // replace with the URL to the PHP script that generates the PDF
