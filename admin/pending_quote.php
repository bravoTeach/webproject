...
$sql = "SELECT * FROM placequote WHERE status = 'in process'";
$result = mysqli_query($conn, $sql);
// -------------------------------
$sql2="SELECT * FROM placequote WHERE status = 'in process'";
...
?>
<div>
<h4>Total Pending Quotes:  <?php echo $inquiry;  ?> </h4>
...
<td>  <a href="download_quote1.php?id=<?= $row["id"];?>">  <img src="<?php echo $imageURL; ?>" alt="" height="50px" width="50px"/></a></td>


<td> <a href="download_quote2.php?id=<?= $row["id"];?>">    <img src="<?php echo $imageURL1; ?>" alt="" height="50px" width="50px"/></a></td>
...
<td><a href="release.php?id=<?= $row["id"];?>"style="background-color:blue;">RELEASE</a></td>
<!--<td><a href="pdf_quotes.php?id=<?= $row["id"];?>"style="background-color:blue;">PDF</a></td>-->
<td><a href="pdf_quotes.php?id=<?= $row['id'] ?>" onclick="generatePDF(event, <?= $row['id'] ?>);" style="background-color: blue;">PDF</a></td>

<script>
function generatePDF(event, quoteId) {
    event.preventDefault(); // prevent the link from navigating to the PDF file directly

    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'pdf_quotes.php?id=' + quoteId); // replace with the URL to the PHP script that generates the PDF
...
<!--<td><a href="delete_quote.php?id=<?= $row["id"];?>">DELETE</a></td>-->
