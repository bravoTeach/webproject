...
$sql = "SELECT * FROM placevector WHERE status = 'in process'";
$result = mysqli_query($conn, $sql);
// -------------------------------
$sql2="SELECT * FROM placevector WHERE status = 'in process' ORDER BY id DESC ";
...
?>
<div>
<h4>Total Pending Vectors:  <?php echo $inquiry;  ?> </h4>
...
<td><a href="release_vector.php?id=<?= $row["id"];?>"style="background-color:blue;">RELEASE</a></td>
<!--<td><a href="pdf_vector.php?id=<?= $row["id"];?>"style="background-color:blue;">PDF</a></td>-->
<td><a href="pdf_vector.php?id=<?= $row['id'] ?>" onclick="generatePDF(event, <?= $row['id'] ?>);" style="background-color: blue;">PDF</a></td>

<script>
function generatePDF(event, vectorId) {
    event.preventDefault(); // prevent the link from navigating to the PDF file directly

    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'pdf_vector.php?id=' + vectorId); // replace with the URL to the PHP script that generates the PDF
