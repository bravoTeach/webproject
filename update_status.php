...
// echo "jibran";
include "conn.php";
$selected_orders = isset($_GET['selected_orders1']) ? explode(',', $_GET['selected_orders1']) : array();
if (!empty($selected_orders)) {
  ...
  $update_query = "UPDATE release_order SET status = 'paid' WHERE orderid IN ($selected_order_ids)";
  
  // Execute the query
  if (mysqli_query($conn, $update_query)) {
      echo "Selected orders updated successfully.";
  } else {
      echo "Error updating selected orders: " . mysqli_error($conn);
  }
}
?>