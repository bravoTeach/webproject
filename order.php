...
<br>
    <section class="midcontent">
        ...
        <div><a href="index.php"><button class="btn btn-primary " >Home</button></a></div>
                ...
                <div><a href="order.php"><button class="btn btn-primary active">Place Order</button></a></div>
...
<select name="required" >
	...
	<option value="EMB-6">EMB-6</option>
	<option value="EMB-8">EMB-8</option>
...
if (empty($_POST['name']) && empty($_POST['required']) && empty($_POST['fabric']) && empty($_POST['placement'])) {
    ...
    $urgent = $_POST['urgent'];
        ...
        if($uploadOk){
            if($image2Inserted){
                $sql = "INSERT INTO placeorder (name,required,height,width,fabric,placement,color,ins,image,image1,urgent,user,email,status) VALUES ('$name1','$required','$height','$width','$fabric','$placement','$color','$ins','$image1','$image2','$urgent','$user','$email1','$status')";
                ...
                $mail->Subject = 'Thanks for Placing Order at.com (OrderID# OR-1'.$last_id.')('.$name1.') ';
        $mail->Body = "<p>Dear ".$_SESSION['name_cus']."</p>
      <p>Thank you for your digitizing order. Your order files will be emailed to you within 12-24 hours. You can also download your files from 'Digitizing Section' at wwwom</p>
    ...
    <p>Best Regards,<br>Team <br>Tel: (915) 266-4883<br>Email: jibranahmed1752@gmail.com<br>order@com<br>URL: www.com</p>";
                ...
                $sql = "INSERT INTO placeorder (name,required,height,width,fabric,placement,color,ins,image,urgent,user,email,status) VALUES ('$name1','$required','$height','$width','$fabric','$placement','$color','$ins','$image1','$urgent','$user','$email1','$status')";
                if(mysqli_query($conn, $sql)){
                ...
                $mail->Subject = 'Thanks for Placing Order at.com (OrderID# OR-1'.$last_id.')('.$name1.') ';
                $mail->Body = "<p>Dear ".$_SESSION['name_cus']."</p>
              <p>Thank you for your digitizing order. Your order files will be emailed to you within 12-24 hours. You can also download your files from 'Digitizing Section' atcom</p>
